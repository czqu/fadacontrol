...
func initDesktopServiceApplication(ctx context.Context, db *conf.DatabaseConf) (*DesktopServiceApp, error) {
	...
	bluetoothController := admin_controller.NewBluetoothController(bluetoothService)
	httpController := admin_controller.NewHttpController(ctx, gormDB, httpService)
	...
	adminRouter := admin_router.NewAdminRouter(bluetoothController, debugController, httpController, systemController, jwtMiddleware, remoteController, unlockController, controlPCController, discoverController, authController)
